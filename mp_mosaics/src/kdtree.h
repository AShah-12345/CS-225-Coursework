...
class KDTree
{
  ...
  public:
    ...
    int select(vector<Point<Dim>>& points, int left, int right, int k, int dim);
    KDTreeNode* buildTree(vector<Point<Dim>>& points, int left, int right, int dim);
    /**
    ...
    void findNearestNeighborHelper(KDTreeNode* node, const Point<Dim>& query, Point<Dim>& nearest, double& bestDist, int depth) const;
