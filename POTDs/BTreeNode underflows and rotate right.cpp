...
bool BTreeNode::rotateRight(unsigned idx, unsigned order) {
    ...
    int original = elements_[idx - 1];
int borrowed = children_[idx - 1]->elements_.back();
elements_[idx - 1] = borrowed;
children_[idx - 1]->elements_.pop_back();
children_[idx]->elements_.insert(children_[idx]->elements_.begin(), original);
